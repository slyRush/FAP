...
$app->post('/login', function() use ($app, $db, $logManager) {
    ...
    $panneau_query = $db->entityManager->panneau("email = ?", $email);
    $panneau = $panneau_query->fetch();

    if( $panneau != FALSE ) //false si l'email de l'panneau n'est pas trouvé
    {
        if (PassHash::check_password($panneau['password_hash'], $password))
        {
            $user = JSON::removeNode($panneau, "password_hash"); //remove password_hash column from $user
            if($user["status"] == 0) //panneau activé
            {
                $logManager->setLog($panneau, (string)$panneau_query, false);
                echoResponse(200, true, "Connexion réussie", $panneau); // Mot de passe utilisateur est correcte
            }
            else
            {
                $logManager->setLog($panneau, (string)$panneau_query, true);
                echoResponse(200, true, "Connexion réussie", $panneau); // Mot de passe utilisateur est correcte
            }
        }
        else
...
$app->post('/register', function() use ($app, $db, $logManager) {
    ...
    $panneau_exist_query = $db->entityManager->panneau("email = ?", $email);
    $panneau_exist = $db->entityManager->panneau("email = ?", $email)->fetch();

    if($panneau_exist == FALSE) //email panneau doesn't exist
    {
        $data = array(
            "name"              => $name,
            "email"             => $email,
        ...
        {
            $logManager->setLog(null, (string)$panneau_exist_query . " / " . buildSqlQueryInsert("panneau", $data), true);
        ...
        else
        {
            ...
            {
                $logManager->setLog(null, (string)$panneau_exist_query . " / " . buildSqlQueryInsert("panneau", $data), false);
                echoResponse(201, true, "Author inscrit avec succès", $insert_panneau);
            }
        }
    }
    else
    {
        if($panneau_exist != FALSE || count($panneau_exist) > 1)
        {
            $logManager->setLog(null, (string)$panneau_exist_query, false);
