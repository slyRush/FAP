...
$app->post('/login', function() use ($app, $db, $logManager) {
    verifyRequiredParams(array('email','password')); // verify required params
        ...
        if (PassHash::check_password($user['password_hash'], $password))
        {
            switch($role)
            {
                ...
                case "5" : //Afficheur
                {
                    ...
                    //$user['fournisseur'] = removeKeyByUser(JSON::parseNotormObjectToArray($supplierAssociated), $supplierAssociated['role_id']);
                    $user = insertKeyValuePairInArray($user, 'fournisseur', removeKeyByUser(JSON::parseNotormObjectToArray($supplierAssociated), $supplierAssociated['role_id']), 10); //insert supplier object after role_id
